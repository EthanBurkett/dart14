...
export default class CommandLoader {
  ...
  private _loadDefaults() {
    const files = getAllFiles(p.join(__dirname, "..", "commands"));

    for (const file of files) {
      const command = file.fileContents.command(this._settings) as Command;
      ...
      this._commands.set(
        file.name[0],
        file.fileContents.command(this._settings)
      );
    }
  }
